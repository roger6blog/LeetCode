...
class Solution(object):
    ...
    def findWords(self, board, words):
        ...
        for word in words:
            trie.insert(word)
        visited = [[False for i in xrange(row)] for j in xrange(col)]
        for x in xrange(col):
            for y in xrange(row):
                self.dfs(board, row, col, visited, x, y, [], trie)


        return self.ans.keys()

    def dfs(self, board, row, col, visited, x, y, currWord, trie):
        if x >= col or x < 0:
            return
        if y >= row or y < 0:
            return
        ...
        if board[x][y] not in trie.child:
            return

        currWord.append(board[x][y])
        visited[x][y] = True
        next = trie.child[board[x][y]]
        if next.isWord == True:
            self.ans["".join(currWord)] = True

        if x != 0:
            self.dfs(board, row, col, visited, x-1, y, currWord, next)
        if x < row:
            self.dfs(board, row, col, visited, x+1, y, currWord, next)
        if y != 0:
            self.dfs(board, row, col, visited, x, y-1, currWord, next)
        if y < col:
            self.dfs(board, row, col, visited, x, y+1, currWord, next)
        visited[x][y] = False
        currWord.pop()
